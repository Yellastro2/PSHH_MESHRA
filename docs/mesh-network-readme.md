...
4. Добавить `MeshRouter` между room/voice протоколами и физическими `NeighborTransport`-ами.
5. Использовать Wi-Fi Aware как основной neighbor transport там, где он доступен.
6. Держать Nearby Connections как fallback и bootstrap/signaling.
7. Перевести chat/control на controlled flooding или route-based relay.
8. Для voice сделать отдельную политику: route selection, bounded relay, без наивного flood каждого Opus frame.
