...
3. Держать voice media-plane отдельно: `NearbyVoiceTransport` использует тот же `NeighborTransport`, но видит только compact `SV` voice frames; MESHRA использует тот же общий 9-байтовый layout с magic `MV`.
...
8. Для voice используется отдельная realtime-политика: bounded playback queue, короткий dedup-cache и отсутствие аудио в event log. MESHRA voice уже flood-ится как compact ephemeral payload с TTL.
