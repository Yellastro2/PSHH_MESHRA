...
`RoomConnectionService` также показывает отдельные уведомления о новых чужих сообщениях только когда приложение не находится на экране. При старте сервиса старая история сообщений не превращается в уведомления. Сервис не использует sticky-восстановление: если Android убил процесс, in-memory `RoomRuntime` уже потерян, поэтому системный restart без явной команды останавливается вместо удержания пустой псевдокомнаты.

При закреплении PTT foreground notification получает текст `Микрофон включён`, action `Откл. микро` и тип foreground service `microphone` вместе с `connectedDevice`. Action останавливает только локальную передачу через общий `RoomRepository`, не отключая пользователя от комнаты.
